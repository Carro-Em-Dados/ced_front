...
export default function EditOrganization({ workshop, setWorkshops }: Props) {
  ...
  const [corporateName, setCorporateName] = useState(workshop.company_name || "");
  const [cnpj, setCnpj] = useState(workshop.cnpj || "");
  const [stateRegistration, setStateRegistration] = useState(workshop.state_registration || "");
  const [municipalRegistration, setMunicipalRegistration] = useState(workshop.municipal_registration || "");
    ...
    <>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} onClose={handleClose} className={styles.modal} size="2xl" scrollBehavior="outside">
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className={clsx("flex flex-col gap-1", styles.modalTitle)}>Editar Oficina</ModalHeader>
              <ModalBody>
