...
export default function EditEventModal({ selectedEvent, events, setEvents, open, setOpen }: Props) {
  ...
  const editSchedule = async () => {
    try {
      ...
      if (currentWorkshop?.google_calendar_id && selectedEvent?.google_event_id)
        await updateGoogleEvent(currentWorkshop.google_calendar_id, selectedEvent.google_event_id, {
  ...
  useEffect(() => {
    if (selectedEvent) {
      ...
      setStart(selectedEvent.start.toTimeString().split(" ")[0].substring(0, 5));
  ...
  return (
    <Modal isOpen={open} className={styles.modal} size="2xl" onOpenChange={() => setOpen(!open)}>
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className={clsx("flex flex-col gap-1", styles.modalTitle)}>Editar evento</ModalHeader>
            <ModalBody className="text-white">
              <div className={clsx(styles.form, "flex flex-col gap-4")}>
                ...
                <div className="flex flex-col gap-2 w-full">
                  <textarea className={styles.modalInput} placeholder="Observação" value={note} onChange={(e) => setNote(e.target.value)} />
                </div>
              </div>
            </ModalBody>
            <ModalFooter>
              <DeleteEventModal events={events} setEvents={setEvents} selectedEvent={selectedEvent} onClose={onClose} />
              <Button color="success" className={styles.modalButton} onPress={editSchedule}>
                Salvar
              </Button>
            </ModalFooter>
          </>
