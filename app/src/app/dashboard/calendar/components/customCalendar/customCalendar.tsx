...
export default function CustomCalendar(
	...
	const fetchMaintenances = async () => {
		...
		const scheduledMaintenanceIds = schedulesSnapshot.docs.map(
			(doc) => doc.data().maintenance
		);
		...
		const fetchedMaintenances = await Promise.all(
			maintenancesSnapshot.docs.map(async (docSnapshot) => {
				...
				const vehicleSnapshot = await getDoc(
					doc(db, "vehicles", maintenance.car_id)
				);
				...
				if (clientSnapshot.exists()) {
					...
					const appUserSnapshot = await getDoc(
						doc(db, "appUsers", vehicle.owner)
					);
				...
				return {
					id: docSnapshot.id,
					...maintenance,
					name: `${ownerName} - ${vehicle.license_plate}`,
				};
			})
		);
		...
		const unscheduledMaintenances = fetchedMaintenances.filter(
			(maintenance) =>
				maintenance && !scheduledMaintenanceIds.includes(maintenance.id)
		);

		setMaintenances(unscheduledMaintenances as Maintenance[]);
	};
